...
class OverallQualityScore(BaseModel):
    ...
    research_depth: int = Field(
        description=(
            "Integer score 1-5 showing whether the report meets the provided criteria" " (1 = doesn't meet at all, 5 = meets all criteria)."
        )
    )
...
def eval_overall_quality(inputs: dict, outputs: dict):
    ...
    if isinstance(eval_model, ChatAnthropic):
        user_input_content = [{"type": "text", "text": user_input_content, "cache_control": {"type": "ephemeral", "ttl": "1h"}}]
    eval_result = cast(
        OverallQualityScore,
...
def eval_relevance(inputs: dict, outputs: dict):
    ...
    eval_result = cast(
        ...
        eval_model.with_structured_output(RelevanceScore).invoke(
            [{"role": "system", "content": RELEVANCE_PROMPT.format(today=get_today_str())}, {"role": "user", "content": user_input_content}]
        ),
    )
...
def eval_structure(inputs: dict, outputs: dict):
    ...
    eval_result = cast(
        StructureScore, eval_model.with_structured_output(StructureScore).invoke([{"role": "user", "content": user_input_content}])
    )
...
def eval_correctness(inputs: dict, outputs: dict, reference_outputs: dict):
    ...
    eval_result = cast(
        CorrectnessScore, eval_model.with_structured_output(CorrectnessScore).invoke([{"role": "user", "content": user_input_content}])
    )
...
def eval_completeness(inputs: dict, outputs: dict):
    ...
    user_input_content = COMPLETENESS_PROMPT.format(
        user_question=query, research_brief=research_brief, report=final_report, today=get_today_str()
    )
    if isinstance(eval_model, ChatAnthropic):
        user_input_content = [{"type": "text", "text": user_input_content, "cache_control": {"type": "ephemeral", "ttl": "1h"}}]

    eval_result = cast(
        CompletenessScore, eval_model.with_structured_output(CompletenessScore).invoke([{"role": "user", "content": user_input_content}])
    )

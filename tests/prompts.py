OVERALL_QUALITY_PROMPT = EVALUATION_PROMPT = (
    """
You are an expert evaluator tasked with assessing the quality of research reports. Please evaluate the provided research
report across multiple dimensions, provide scores, and offer a comprehensive assessment.
...
Evaluation Process:
1. Begin by analyzing each dimension separately. Wrap your analysis for each dimension in <dimension_analysis> tags.
   For each dimension:
...
CORRECTNESS_PROMPT = """
You are evaluating the correctness of a research report that was generated by a research agent.
...
RELEVANCE_PROMPT = """
You are evaluating the relevance of a response to a user's input question.
Please assess the answer against the following criteria, being especially strict
about section relevance.

1. Topic Relevance (Overall): Does the response directly address the user's
input questions thoroughly?

2. Section Relevance (Critical): CAREFULLY assess each individual section for
relevance to the main topic:
   ...
   - Flag any sections that seem tangential, off-topic, or only loosely connected
     to the main topic
...
4. Overall Quality: Is the response well-researched, accurate, and
professionally written?

Evaluation Instructions:
- Be STRICT about section relevance - ALL sections must clearly connect to the
  primary topic
- You must individually mention each section by name and assess its relevance
- Provide specific examples from the response to justify your evaluation for
  each criterion
...
STRUCTURE_PROMPT = """
You are evaluating the structure and flow of a response to a user's question.
...
- Be in the correct format for the user's question
   - For example, if the user asks for a list of N items, the report should be
     a list of N items, no more, no less. This is crucial.
   - For example, if the user asks for a comparison of X and Y, the report
     should have a section about X, a section about Y, and a comparison of
     the two.
- Have a logical flow from one section to the next
- Be appropriate for the user's question
- Use structural elements (e.g., headers, tables, lists) to effectively convey
  information
- Have properly formatted section headers (e.g., # for title, ## for sections,
  ### for subsections)
</Rubric>

<Instruction>
- Compare the output against the user's question, is it in the format that the
  user asked for?
- Identify any sections that are not logically connected to the user's question
- Identify any sections that are not logically connected to the previous and
  next sections
- Reason about whether the structure is the best structure for the user's
  question
</Instruction>
...
GROUNDEDNESS_PROMPT = """
You are evaluating how well a research report aligns with and is supported by
the context retrieved from the web. Your evaluation should focus on the
following criteria:
...
- Claims are often made where you see citations. You can check against the
  cited source as well.
</Instruction>
...
COMPLETENESS_PROMPT = """
You are evaluating the completeness of a research report. Your evaluation should focus on the following criteria:

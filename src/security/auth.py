...
async def get_current_user(authorization: str | None) -> Auth.types.MinimalUserDict:
    ...
    if not supabase:
        raise Auth.exceptions.HTTPException(status_code=500, detail="Supabase client not initialized")
    ...
    try:
        ...
        if not user:
            raise Auth.exceptions.HTTPException(status_code=401, detail="Invalid token or user not found")

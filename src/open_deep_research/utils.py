...
async def summarize_webpage(model: BaseChatModel, webpage_content: str) -> str:
    ...
    try:
        ...
        summary = await asyncio.wait_for(
            model.ainvoke([HumanMessage(content=prompt_content)]), timeout=60.0  # 60 second timeout for summarization
        )
...
async def fetch_tokens(config: RunnableConfig) -> dict[str, Any]:
    ...
    Returns:
        Valid token dictionary, or None if unable to obtain tokensget
    """
...
async def load_mcp_tools(
    ...
    mcp_server_config = {"server_1": {"url": server_url, "headers": auth_headers, "transport": "streamable_http"}}
    ...
    for mcp_tool in available_mcp_tools:
        ...
        if mcp_tool.name in existing_tool_names:
            warnings.warn(f"MCP tool '{mcp_tool.name}' conflicts with existing tool name - skipping")
            continue
...
async def get_all_tools(config: RunnableConfig):
    ...
    if configurable.rag_tool and configurable.rag_source == RAGSource.SEC:
        # rag_tool_structured = StructuredTool(**configurable.rag_tool)
        tools.append(configurable.rag_tool)

    if configurable.rag_tool and configurable.rag_source == RAGSource.DART:
        tools.append(configurable.rag_tool)

    if configurable.rag_tool and configurable.rag_source == RAGSource.WEB:
        tools.append(configurable.rag_tool)

    if configurable.internal_files_tool and configurable.internal_files == InternalFiles.YES:
        tools.append(configurable.internal_files_tool)
...
def get_api_key_for_model(model_name: str, config: RunnableConfig):
    ...
    # print("Can get os: ", os.getenv("GET_API_KEYS_FROM_CONFIG"))
...
PERPLEXITY_SEARCH_DESCRIPTION = (
    "A search engine powered by Perplexity AI. Returns AI-generated answers " "along with trusted sources, URLs, and snippets."
)

perplexity_semaphore = asyncio.Semaphore(5)


@tool(description=PERPLEXITY_SEARCH_DESCRIPTION)
async def perplexity_search(
    queries: List[str],
    max_results: Annotated[int, "Maximum number of sources to return"] = 5,
    model: Annotated[str, "Perplexity model to use"] = "sonar-pro",
) -> str:
    """Fetch and summarize search results from the Perplexity API.
    ...
    Returns:
        A formatted string containing AI answer + summarized sources.
    """
    print("perplexity called", " ".join(queries))

    api_url = "https://api.perplexity.ai/chat/completions"
    headers = {
        "Authorization": f"Bearer {Secrets.get(SecretKeys.PERPLEXITY_API_KEY)}",
    ...
    class SourceAnswers(BaseModel):
        ...
        title: str = Field(description="Prepare short and concise title of the source")

    class Answers(BaseModel):
        document_type: str = Field(description="Type of the document")
        answers: List[SourceAnswers]

    async def fetch_query(session: aiohttp.ClientSession, query: str) -> str:
        async with perplexity_semaphore:
            payload = {
                ...
                "messages": [
                    {
                        "role": "system",
                        "content": """You are a helpful assistant. Answer only with non-financial company information
                     (e.g., management, products, services, strategy, market size, market share, industry positioning)."
                     " Do NOT include financial metrics such as revenue, EBITDA, EPS, net "
                     "income, cash flow, margins, or other accounting KPIs.
                     Provide answer from each source separately with two headings for all URLs.
                     If the answer is not present for any url source, simply return empty text.
                     Document type is always 'Web Search'""",
                    },
                    {"role": "user", "content": query},
                ],
                "response_format": {"type": "json_schema", "json_schema": {"schema": Answers.model_json_schema()}},
                "search_domain_filter": ["-wikipedia.org", "-finance.yahoo.com", "-youtube.com"],
            ...
            try:
                async with session.post(api_url, headers=headers, json=payload, timeout=60) as response:
                    if response.status != 200:
                        ...
                        return f"Error {response.status} for query '{query}': {text}\n\n"

                    data = await response.json()
                    return data["choices"][0]["message"]["content"]
            #                 answer = data["choices"][0]["message"]["content"]

            #                 output = f"AI Answer (for query: {query}):\n{answer}\n\n"
            ...
            #                         snippet = c.get("snippet", "No snippet")
    ...
    # Run queries in parallel but controlled with semaphore
    async with aiohttp.ClientSession() as session:
        results = await asyncio.gather(*(fetch_query(session, q) for q in queries))

    return "\n\n---\n\n".join(r for r in results)


def list_available_tools(tools_list):
    prompt = ""
    if "internal_files" in tools_list:
        prompt += (
            "**company_internal_files_search**: Source for company-official documents "
            "stored internally"
            " (PDFs, investor decks, spreadsheets, data rooms)."
            " Use for both financials and officially disclosed non-financials.\n"
        )
    if "rag_tool" in tools_list:
        prompt += (
            "**company_reports_search**: Source for company-official documents"
            " (annual & quarterly filings, investor reports hosted internally)."
            " Use this first for all financials and company-disclosed facts. Use '{language}' language to query.\n"
        )

    prompt += (
        "**web_search** — Web search for industry size, market positioning, and competitors along with relevant KPIs;"
        " may also fetch general company profile (mission/vision, product overview) but never numeric"
        " financials. Also use to discover company-specific KPI vocabulary,"
        " then query those terms in rag_tool. When using web_search, "
        "provide one line company info for effective search.\n"
    )
    prompt += "**think_tool**: For reflection and strategic planning during research"
...
class CitationManager:
    def __init__(self, source_content):
        pairs = re.findall(r"\[([^\]]+)\]\(([^)]+)\)", source_content)

        self.citation_map = {f"T{i}": (title, url) for i, (title, url) in enumerate(pairs, 1)}

        self.used = {}
        self.counter = 1

    def replace_titles_with_placeholders(self, text: str) -> str:
        """Replace [title] with [Tn] before sending to LLM."""
        for key, (title, _) in self.citation_map.items():
            text = text.replace(f"[{title}]", f"[{key}]")
        return text

    def process_output(self, text: str) -> str:
        """Replace [Tn] with numbered references after LLM output."""
        for key, (title, url) in self.citation_map.items():
            if f"[{key}]" in text:
                if key not in self.used:
                    self.used[key] = self.counter
                    self.counter += 1
                num = self.used[key]
                text = text.replace(f"[{key}]", f"[{num}]({url})")
        return text

    def build_references(self):
        """Build references list at the end."""
        refs = "\n\n".join(
            f"[{num}] [{title}]({url})"
            for key, num in sorted(self.used.items(), key=lambda x: x[1])
            for title, url in [self.citation_map[key]]
        )
        urls = [url for key, num in sorted(self.used.items(), key=lambda x: x[1]) for _, url in [self.citation_map[key]]]
        return "\n\n# References\n" + refs, urls


def template_to_text(template_obj: list) -> str:

    lines = []
    for section in template_obj:
        name = section.get("name", "").strip()
        description = section.get("description", "").strip()

        lines.append(f"**{name}**")

        desc_lines = [f"   - {line.strip()}" for line in description.split("\n") if line.strip()]
